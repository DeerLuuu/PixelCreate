...
| 5 | **InputServer** 🟡 两片已抽出：`servers/input.ts`（策略与算术）+ `servers/gesture.ts` 的 `TapMachine`（轻点序列状态机）；触点会话与动作体待搬 | 手势状态机：指针生命周期、双指、长按、PC 修饰键 | 当前手势会话 | Tool/Selection/Viewport、`app/gestures` | 不碰像素 |
...
| ↳ | **状态：🟡 部分完成**（2026-09-14）：`RenderServer`（含**平铺重绘区域**与**渲染调试记录**）/ `ViewportServer` 已落地（`src/servers/`）；继续拆的是手势状态机（P5）与 `View` 持有的视口字段 | — | 全量测试 4063 条 ALL PASS；`check-bundle` 通过 |
| **P5** | `InputServer` | 手势状态机真正搬出 View | 手势 / PC 模式 / 多球互斥用例全绿 |
| ↳ | **状态：🟡 两片完成**（2026-09-14）：①`servers/input.ts` 策略与算术（鼠标按键意图、pinch 解算、四指判定、长按策略、点击容差，+47 断言）；②`servers/gesture.ts` 的 `TapMachine` 轻点序列状态机（单击/双击边距·画布·换画布/三击/双指双击，+44 断言，并**发现画布内三击够不到**的既有问题）；待搬：触点会话状态（`pointers` / `pinchBase` / `fourSeen` …）与 `onDown/onMove/onUp` 的动作体 | — | 全量测试 4063 条 ALL PASS |
...
| P5 InputServer | 🟡 两片完成（`src/servers/input.ts` 策略与算术 + 47 断言；`src/servers/gesture.ts` 的 `TapMachine` 轻点序列 + 44 断言；触点会话与动作体待搬） |
| P6 DocumentServer | ⬜ |
| P7 SignalHub | ⬜ |
| P8 AiServer | ⬜ |
