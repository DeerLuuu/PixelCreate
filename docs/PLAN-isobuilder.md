...
`T` 取 **4 / 8 / 16 / 32**（都能被 4 整除 ⇒ 全部落在整数像素上）：

| T | 顶面菱形 | 每单位高度 | 1×1×1 立方体外框 | 适用 |
|---|---|---|---|---|
| 4 | 4×2 px | 2 px | 4×4 px | 图标 / 迷你物件（4px 档把画布放大来用，抓手半径会自动收窄） |
| 8 | 8×4 px | 4 px | 8×8 px | 小物件 |
...
**v1 已落地（2026-09-13）**——引擎 + 模式 + 画布直接操作 + 参数条都跑通了：

| 步骤 | 状态 |
|---|---|
| `engine/iso.ts`：2:1 几何（行宽模板）+ 六个形状 + `isoRender`（三面着色 / 只画可见面 / 接触阴影 / 1px 描边 / 稳定锚点 `originAt`） | ✅ |
| `tests/iso.test.ts`：黄金值（立方体外框与三面像素数、遮挡顺序、行凸不变量、形状不变量、参数夹取、32³ 性能上限） | ✅ |
| `Session`：`isoOn` / `isoOrigin` / `enterIso` / `exitIso` / `setIsoPref` / `setIsoOrigin` / `isoStats` / `isoGenerate`（`layer` 一条像素差分；`new` 用整档快照把「新建图层 + 写像素」合成一条 undo） | ✅ |
| `View`：`drawIsoMode()`（2:1 栅格 → 半透明预览 → 足迹虚线 → 抓手 → 尺寸浮标）、`isoHandles()`、`isoHitAt()`（半径随密度收窄、高度抓手优先）、`isoDragTo()`（四角改宽深 / 黄块调高 / 整块移动，全部吸附栅格） | ✅ |
| `IsoBar`（`src/ui/iso.tsx`）：形状 chips / 宽深高 / 图块 4·8·16·32 / 实时读数（越界标红）/ 折叠外观（颜色模式、三面颜色、明暗、阴影、描边、形状专属参数）/ 生成 / 生成到新图层 / 完成 | ✅ |
| 入口：**魔法球「等距图形」**（按用户要求；画布球不放）+ 主菜单 | ✅ |
| `tests/isoui.test.ts`：模式进出不改像素、生成一条历史且 undo 干净、四角/高度/整块拖动的语义、吸附、越界拒绝、锁定拒写、生成到新图层、参数持久化、三面颜色三种模式、模式期间不落笔迹 | ✅ |
| docs/API.md §6d + §17.1、README 功能表、更新日志 | ✅ |

**对齐修正（2026-09-13 真机反馈「生成的图形没有与网格对齐」）**：

| 项 | 口径 |
|---|---|
| `originAt` | 从「格 (0,0) stamp 左上角」改成**顶面菱形的顶点**（`g0.ox + c`）。stamp 从 `-T/2` 起画，旧锚点比顶点偏左 `T/2`，画布上的栅格 / 足迹虚线 / 抓手整体偏左半格 —— 那正是「没对齐」的来源 |
| 栅格节点 | `isoOnLattice()`：`x` 是 `T/2` 的整数倍、`y` 是 `T/4` 的整数倍**且同奇偶**；`isoSnapOrigin()` 在 ±1 格候选里取最近的合法节点（分别对 x/y 取整会吸到 `(0, T/4)` 这种格子边缘中点） |
| 落点 | `isoPlaceOrigin()`：越界收紧只能沿等距轴挪格（`+x` 一格 = `(+T/2, +T/4)`），早先按 x/y 各自加减 `T/2` / `T/4`，挪完就离开栅格 |
| 30° 参考网格 | iso 模式期间 `drawIsoGuide()` 不画：30° 与 2:1 不可能重合，两套网格并存只会让人以为「没对齐」 |
| 回归 | `tests/iso.test.ts` 的 `iso.align.*`（顶点 = 锚点、顶面菱形逐行对上 `isoDiamondRows`）、`iso.snap.*`、`iso.place.*`；`tests/isoui.test.ts` 的拖动按格断言 |
| 图标 | 每个功能入口一个专属 SVG 并登记在 `src/ui/feature-icons.ts`（iso = `i-iso`），见 docs/API.md §17.5 |

**观感与口径都已肉眼确认**（无头 Edge 实测：进模式 → 拖抓手 → 生成 1474 px → 退出后覆盖层干净）。

**下一步（按优先级）**：
1. 首次进入的三步就地提示 + `guide.ts` 一步真操作演示（§4.6）；
2. 「重编辑上一次」（记住最后一次生成的参数，改参数原地重生成，§4.5）；
3. v2：方向投影阴影（8 档光照）、左右视角、棱柱 / 多段柱础等更多形状、PC 快捷键；
4. v3：三视图高级模式（视觉外壳）、**逐帧渲染＝等距动画**、多画布实时联动、6 向、精灵表导出。
