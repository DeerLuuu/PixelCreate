...
| 令牌 | 值 | 用途 |
|---|---|---|
| `--surface-orb` / `--surface-orb-on` | `#2b2f3e` / `#343a52` | 浮动球 |
| `--surface-item` / `--surface-item-active` | `#262b3a` / `#3d445c` | 环形菜单项 |
| `--line-3` | `#565c74` | 浮动球边框 |
| `--hud-bg` | `rgba(16,17,22,.78)` | 画布 HUD 底（色块框/关闭钮） |
| `--hud-bg-2` | `rgba(21,23,32,.94)` | HUD 胶囊 |
| `--hud-bg-2-dim` | `rgba(21,23,32,.8)` | HUD 胶囊（弱） |
| `--hud-bg-3` | `rgba(21,23,32,.84)` | 对称提示 |
| `--hud-bg-4` | `rgba(21,23,32,.5)` | 对称提示（只读） |
| `--hud-tip` | `rgba(13,15,22,.96)` | 长按提示气泡 |
| `--hud-deep` | `#101116` | 预览窗/时间线胶囊底 |
| `--dock-bg` / `--dock-bg-open` / `--dock-bg-armed` | `rgba(20,22,32,.55)` / `rgba(15,17,25,.92)` / `rgba(30,36,52,.9)` | 停靠条 |
