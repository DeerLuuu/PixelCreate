...
export function testSelOrb(): void {
  ...
  const p4 = page("selPage4");
  // selPage2 的「进自由变形子页」导航项定义在 selWarpNav 里（静态扫描看不到），手工补上
  if (!p2.ids.includes("sel-more-tools")) {
    p2.ids.push("sel-more-tools");
    p2.max += 1;
  }

  // 1) 每页都要在一屏内放得下
  for (const [name, p] of [["page1", p1], ["page2", p2], ["page3", p3], ["page4", p4]] as const) {
    ok("selorb." + name + ".fits", p.max <= MAX_PER_PAGE, name + "=" + p.max);
    ok("selorb." + name + ".has-items", p.ids.length >= 3, name + "=" + p.ids.join(","));
  }

  // 2) 触屏上依赖的全部条目都在某一页里
  //    （上一版新增的自由变换四个入口 + 复制/剪切/粘贴/粘贴为新图层/为新画布）
  const mobile = new Set([...p1.ids, ...p2.ids, ...p3.ids, ...p4.ids]);
  const must = [
  ...
  // 自由变形（四点 / 网格）与「完成 / 还原 / 半像素吸附」必须在同一页（点进去就能接着点）
  ok("selorb.warp-one-page", p3.ids.includes("selWarpQuad") && p3.ids.includes("selWarpMesh")
    && p3.ids.includes("selWarpDone") && p3.ids.includes("selWarpRevert"), p3.ids.join(","));
  ok("selorb.warp-half-on-warp-page", p3.ids.includes("selWarpHalf"), p3.ids.join(","));
  // 自由变换的 sticky 开关行（替代 Shift / Ctrl / Alt）在「变形」页
  ok("selorb.xform-chips", ["selXfAspect", "selXfAngleSnap", "selXfGridSnap", "selXfCopy", "selXfPivot"]
    .every((id) => p2.ids.includes(id)), p2.ids.join(","));
  ...
  const all = [...p1.ids, ...p2.ids, ...p3.ids, ...p4.ids];
  eq("selorb.no-duplicate-ids", all.filter((id, i) => all.indexOf(id) !== i), []);
  ok("selorb.pc-view-merges", pcView.includes("selPage1") && pcView.includes("selPage2")
    && pcView.includes("selPage3") && pcView.includes("selPage4"), pcView.slice(0, 80));
  ...
  ok("selorb.anchor-more-more", appSrc.includes('guide: "sel-more-more"'));
