...
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | **架构现状与 Server 化改造**（盘点 + 目标设计）：分层与规模实测、数据模型与不变量、运行时主链路、能力域、实测依赖现状与三处硬伤、15 个 server 的职责与所有权、算法类清单、目标依赖图、信号总线、绞杀者迁移分期 P0–P8、红线、量化目标 |
