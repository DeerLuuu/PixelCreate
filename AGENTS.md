...
标签细节（同日追加）：循环按钮按模式换图标（`i-loop` / `i-loop-once` / `i-loop-pingpong` / `i-loop-reverse`）；播放中点其它标签的帧＝`Session.retargetPlay` 切换正在播的动画（点到无标签的帧＝回到整条时间轴）；重叠标签用 `tagLanes` 分层显示（每条一行，时间轴的标签区行数随之变化）；标签条手势＝轻点播放（`Session.tagPlay`）、拖左右边缘改范围（拖完才提交，一次拖动一条历史）、右键/长按打开编辑器。
