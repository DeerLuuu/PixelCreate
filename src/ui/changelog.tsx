...
export const BUILD_TAG = "72ff2b0";
...
export const CHANGELOG: ClgVersion[] = [
  {
    ...
    items: [
      ...
      it("add", "自动保存改成多版本：默认保留最近 5 版（设置 → 数据 → 保留版本数，可调 1–12 版），每版都记下保存时间、大小、工程名与是定时保存还是切到后台时存的。内容没变时不会占一个新版本，所以放着不动不会把版本耗光。以前的自动保存会原样变成第一版，不会丢。", "Autosave now keeps several versions: five by default (Settings → Data → Versions kept, 1 to 12), each stamped with its time, size, project name and whether it was a scheduled save or one taken when the app was hidden. An unchanged project does not take a new version, so leaving the app idle never uses them up, and whatever older builds had autosaved becomes the first version instead of being lost."),
      it("add", "自动保存历史面板（主菜单「自动保存历史」，或设置 → 数据）：逐版列出，可以恢复某一版、把它导出成 .pxc、或删掉。恢复会先用这一版替换当前打开的工程，替换前会问一次。", "An autosave history panel (main menu → Autosave history, or Settings → Data) lists every version and can restore one, export it as a .pxc file or delete it. Restoring replaces the project you have open and asks first."),
      it("add", "启动恢复提示：上次没正常退出（崩溃、被系统杀掉）时，启动后除了自动恢复最近一次的自动保存，还会弹出一个面板说明这件事，并列出所有可回退的版本。切到后台或正常退出不算崩溃，不会弹。", "A recovery prompt on launch: if the app did not close cleanly last time (a crash or the system killing it), it still restores the newest autosave and then shows a panel explaining that, listing every version you can go back to. Switching away or closing normally is not treated as a crash and shows nothing."),
      it("imp", "自动保存占用的空间有上限：历史版本加起来超过容量时会先淘汰最旧的版本，最新一版永远保留；万一空间实在不够，会尽量覆盖最旧的那一版来腾地方，而不是把历史删光。", "Autosave storage is capped: once the versions together pass the limit the oldest ones go first and the newest is always kept. If space truly runs out it tries to overwrite the oldest version rather than wiping the history."),
    ],
  },
  {
    v: "1.1.1.8",
