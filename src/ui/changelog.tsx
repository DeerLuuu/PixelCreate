...
export const CHANGELOG: ClgVersion[] = [
  {
    ...
    items: [
      ...
      it("imp", "历史记录改为整个工程共用一条：不再按画布分开保存，撤销/重做按全局顺序作用到对应的画布（切画布不再改变撤销栈），「过程回放」也变成全局回放——一次把所有画布的操作按顺序播完；关闭某张画布只会丢弃它自己的步骤，其余画布的历史保留；工程文件里只存一条历史（每一步记录它属于哪张画布），旧版本按画布分开保存的历史在打开时会按画布顺序合并", "History is now one shared stack for the whole project instead of one per canvas: undo/redo walk the global order and apply to the canvas each step belongs to (switching canvases no longer changes the stack), and Replay is global too — every canvas' operations play back in one sequence. Closing a canvas only drops its own steps, the rest of the history stays, and the project file stores a single history where each step names its canvas; projects saved by older builds with per-canvas histories are merged in canvas order on load"),
