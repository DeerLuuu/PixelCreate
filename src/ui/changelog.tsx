...
export const CHANGELOG: ClgVersion[] = [
  {
    ...
    items: [
      ...
      it("add", "新增平铺画布（设置 → 画布与网格 → 平铺画布）：在中心画布四周显示 3×3 的副本，只有中心可编辑，画无缝瓦片/背景时能直接看到接缝；支持「平铺重复」与「镜像平铺」两种拼法，中心格有蓝色描边标识", "New tiled canvas (Settings -> Canvas & Grid -> Tiled canvas): a 3x3 set of copies around the centre canvas, only the centre is editable, so you can see the seams while painting seamless tiles/backgrounds. Two layouts: plain repeat and mirrored repeat, with a blue outline marking the editable tile"),
    ],
  },
  {
    v: "1.0.7.10",
