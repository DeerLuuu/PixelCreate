...
export const CHANGELOG: ClgVersion[] = [
  {
    ...
    items: [
      ...
      it("fix", "修复在引用画布上绘画没有实时预览的 bug（笔、图形、橡皮擦）：引用层的笔迹被重定向到源画布，源画布的像素版本号变了，但视图的引用层缓存只在 changed() 时失效，而画的过程中不会走 changed()——现在像素版本号写进缓存键与画布间缓存键，笔画期间引用层实时刷新", "Fixed the missing live preview when painting on a reference layer (pencil, shapes, eraser): the stroke is redirected to the source canvas, whose pixel version changed, but the reference cache only invalidated on changed(), which a live stroke never fires — the version is now part of both cache keys, so reference layers refresh during the stroke"),
