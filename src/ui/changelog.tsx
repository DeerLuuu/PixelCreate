...
export const BUILD_TAG = "57d5ada";
...
export const CHANGELOG: ClgVersion[] = [
  {
    ...
    items: [
      ...
      it("add", "等距图形（魔法球 →「等距图形」）：同一条参数条上选好形状、宽深高与每格像素，画布上就会出现这个等距体的半透明预览，拖四个菱形角抓手改宽深、拖黄色方块调高度、拖整块移动，全部吸附 2:1 栅格，松手才落笔。六种形状是立方体、楼梯、楔形、圆柱、金字塔、空心框，每格可选 8 / 16 / 32 像素（1×1×1 立方体正好是 8×8 / 16×16 / 32×32 像素）；楼梯级数与方向、楔形坡向、圆柱半径与空心、金字塔平顶、空心框壁厚都在「外观」里。", "Iso shapes (magic ball → Iso shapes): pick a shape, its width, depth, height and the pixels per cell in one bar, and a translucent preview of that isometric solid appears on the canvas. Drag the four diamond corners to resize the footprint, the yellow square to raise it, or the block itself to move it — everything snaps to the 2:1 grid and nothing is committed until you let go. The six shapes are cube, steps, wedge, cylinder, pyramid and frame, at 8 / 16 / 32 pixels per cell (a 1×1×1 cube is exactly 8×8 / 16×16 / 32×32 px). Steps count and direction, wedge slope, cylinder radius and hollow, pyramid flat top and frame wall thickness all live under Look."),
