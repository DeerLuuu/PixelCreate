...
export const FEATURE_ICONS = {
  ...
  /**
   * 应用内助手的**参考图附件条**（`ui/AiPanel.tsx`）：两个挂图入口**并排**出现，
   * 所以必须能一眼分清（真机反馈过「并排的入口借同一个图标根本分不清」）。
   *
   * 两个值都是**既有的、语义对得上的**图标，因此这一组**不新造 SVG**：
   *   · `ref` = **用当前参考图**（取会话里那张参考图 `SESSION.refImg`）→ `i-ref`
   *     （两张叠图 + 虚线关联，本来就是「参考图」语义；仓库里只有画布球用它，
   *     与助手的浮窗**不会同屏**）；
   *   · `file` = **选择图片文件**（从磁盘挑一张）→ `i-import`（向上箭头 + 底线，
   *     与主菜单「导入图片」同一个 —— 那一条在主菜单里，也不会与助手浮窗同屏）。
   *
   * 两条都满足 §5.2 的「同屏不得重复」；这一组内部两个值互不相同，
   * `tests/icons.test.ts` 的组内唯一与存在性照样成立。
   */
  aiAttach: {
    ref: "i-ref",
    file: "i-import",
