...
| 绘制 | 铅笔 / 橡皮 / 油漆桶（含渐变与缝隙填充）/ 取色器 / 喷枪 / 直线 / 矩形 / 椭圆 / 圆 / 多边形 / 折线 / 曲线 / 轮廓填充 / 矩形与套索选区 / 魔棒；**图案笔刷**（10 个内置图案 + 图案库面板，选区或整张画布都能存成图案；图案按画布坐标平铺，透明处不落笔，橡皮配图案＝只擦图案点）|
...
- **电脑模式（1.0.8.5 起，1.0.8.6/1.0.8.7 补齐）**：检测到鼠标自动开启（设置 → 显示与取色可强制开/关）——
  ...
  **双击主球＝切回上一个工具**、**把橡皮小项从工具球里拖到画布上＝临时橡皮**（不切换当前工具）、**动作搜索**（PC `Ctrl+K`，触摸端在主球上方）、**一键从画布生成调色板**、**内描边**（保住最外圈原色、可带透明度混合）、**圆角化**（硬直角削成圆角，细线与斜线安全）、**快捷圆盘**（存储区边的装备槽里装备一个球，按住 F 发动：子项以圆环铺在屏幕正中、隐藏鼠标、
