...
| 播放 | 单次 / 循环 / 乒乓 / 倒流，逐帧时长驱动；**播放范围跟着动画标签走** |
| 动画标签 | **选中多帧一键加成标签**（Aseprite 风格的命名帧区间）：时间轴上方多一条彩色标签，点开可改名 / 改范围 / 换色 / 删除 / **直接播放这一段 / 选中这些帧**；**从标签内的任意一帧开始播放就只循环这一段**，播放起点不在任何标签里时播放整条时间轴；标签随 `.pxc` 与 `.aseprite` 一起存取 |
