...
| 调色板 | HSV 色轮 + HEX + 预设色板；色板 / 画布颜色 / 最近使用三种来源；长按色块全局换色；**去重 / 按色相·明度排序 / 合并预设色板**；`.gpl` 导入导出；**索引色模式**（画笔只落调色板颜色 + 把已有画面映射到调色板） 、**拖动颜色球到画布上直接油漆桶填充**（松手即填，原地松手仍是取色；落在非聚焦画布上会先聚焦再填，只记一条历史）|
