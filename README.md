...
| 动画标签 | **选中多帧一键加成标签**（Aseprite 风格的命名帧区间）：时间轴上方一条彩色标签，点开可改名 / 改范围 / 换色 / 删除 / **直接播放这一段 / 选中这些帧**；**从标签内的任意一帧开始播放就只循环这一段**，播放起点不在任何标签里时播放整条时间轴；**播放中点别的标签的帧＝切换正在播的动画**；**多个标签重叠时自动分层显示**（各占一行，不会互相盖住）；标签随 `.pxc` 与 `.aseprite` 一起存取 |
