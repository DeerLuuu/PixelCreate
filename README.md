...
| 动画标签 | **选中多帧一键加成标签**（Aseprite 风格的命名帧区间）：时间轴上方的彩色标签条 **点一下＝播放这一段**、**拖左右边缘＝直接改范围**、**右键 / 长按＝编辑器**（改名 / 换色 / 删除 / 选中这些帧）；**从标签内的任意一帧开始播放就只循环这一段**，播放起点不在任何标签里时播放整条时间轴；**播放中点别的标签的帧＝切换正在播的动画**；**多个标签重叠时自动分层显示**（各占一行，不会互相盖住）；标签随 `.pxc` 与 `.aseprite` 一起存取 |
