...
| 参考图发给模型 | **助手可以把你挂的一张参考图和那句话一起发给模型（图像理解 / vision）**：助手浮窗输入框上方多了附件条，两个入口 —— 「用当前参考图」（直接取已经导入的那张参考图）与「选择图片文件」（PNG / JPEG / WebP / GIF）；图会先按最长边 768px 夹一次（**只缩不放**，整数倍缩小走最近邻、其余走盒式平均），再编成 PNG 发出去，附件条上写明「原尺寸 → 现在尺寸 · 体积」；**发送后附件保留**，换个说法再问同一张图不用重新挂（最小化再打开也还在），只有点「移除」才清；**模型不支持看图时在发送前就拦下**并写明该换哪个（DeepSeek 里 `deepseek-flash` 支持图像理解、`deepseek-v4-pro` 不支持，其他模型按「允许发送 + 一句提示」处理）；图有软硬两道体积线（超过 512 KiB 在附件条上提示、超过 768 KiB 拦下不发，因为本机壳对请求体的上限是 1 MiB） |
...
│               ai-turn（回合事务）、ai-rpc（AI 协议路由）、ai-serve（本地服务生命周期）、ai-presets（厂商预设：DeepSeek 默认）、ai-chat（应用内助手的整轮循环 + function calling）、ai-vision（参考图 / 视觉输入：尺寸夹取 · 体积判定 · 能力门控）
