...
npm test               # 引擎 / 逻辑回归测试（2924 条断言，无 DOM 依赖；跑完末尾会打印总数）
...
- **引擎层零 DOM**：`engine/`、`app/`、`tools/` 全部可在 Node 下测试（2924 条断言跑在纯数据上，含 UI 控件与令牌契约；`npm test` 末尾会打印条数）。
...
- **电脑模式（1.0.8.5 起，1.0.8.6/1.0.8.7 补齐）**：检测到鼠标自动开启（设置 → 显示与取色可强制开/关）——
  ...
  **选区自由变换在 PC 上用「双层同心圈」**（内圈缩放 / 角外圈旋转 / 边中点外圈斜切，悬停会把可抓的那圈点亮；触屏则是独立抓手）、
